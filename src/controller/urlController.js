...
const createUrl = async function (req, res) {
    try {
        ...
        if (cacheData) {
            return res.status(200).send({ status: true, msg: "Shorten Url already Generated (From cache Memory)", data: cacheData })
        } else {

            //If data present in Database
            let checkDb = await urlModel.findOne({ longUrl: data.longUrl }).select({ __v: 0, createdAt: 0, updatedAt: 0 })
            if (checkDb) {
                await SET_ASYNC(`${data.longUrl}`, JSON.stringify(checkDb))
                return res.status(200).send({ status: true, msg: "Shorten Url already Generated (From Database)", data: checkDb })
            }
        }
...
const getUrl = async function (req, res) {
    try {
        ...
        if (data) {
            ...
            if (!urlData) {
                return res.status(404).send({ status: false, msg: "URL Not Found, Please Enter Valid Shorten UrlCode." })
            }
